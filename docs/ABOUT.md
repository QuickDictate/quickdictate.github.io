...
A hand-crafted marketing and product showcase site for QuickDictate, a Windows tray app for speech-to-text. Single HTML file with no build process, deployed on GitHub Pages. Explains the app's capabilities (cloud or offline models), unique positioning (no subscription, free for personal use, source available under PolyForm Noncommercial), and links to downloads and source.
...
- The privacy section is a deliberate positioning decision baked into the copy, not just a feature description, so it should never be genericized in a rewrite - but it must stay TRUE, and on 2026-09-10 it was not. The page said "zero telemetry" in eight places (both meta descriptions, a hero badge, the visible FAQ, the FAQPage JSON-LD, llms.txt, and llms-full.txt twice) while the app had shipped an opt-in anonymized usage rollup in v0.9.0 and had always made a daily update check carrying an anonymous install id. The app's own `.github/SECURITY.md` documented both honestly, so the marketing copy was contradicting the security policy of the thing it was selling. It now says "Private by default" and names exactly what leaves: nothing dictated, one daily update check with a random install id (switchable off), and an anonymized rollup that is off until you turn it on. ⛔ Do not "restore" the stronger wording - the claim was retired for being false, not for being too bold. Any new app feature that talks to a network is a trigger to re-read this section against SECURITY.md. anchors: `index.html:1095`
...
_Generated by `odin codex about --publish quickdictate-github-io` on 2026-09-10 from a Codex dossier stamped 2026-09-10. Regenerate after the product moves; `odin codex about` reports drift._
<!-- odin:about GENERATED END sha=6d59c6479199 -->
