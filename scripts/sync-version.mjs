// Keep the version and the download links baked into index.html equal to the
// latest published release.
//
// WHY THIS EXISTS. The page corrects itself in the browser: a small script at the
// bottom of index.html fetches /releases/latest and rewrites the version labels,
// the download hrefs and the JSON-LD after load. That is enough for a human with a
// working network. It is not enough for anyone else.
//
// Everything that does not run JavaScript reads the STATIC file: search crawlers,
// the AI answer engines this site carries schema.org markup for in the first place,
// link unfurlers, and every visitor whose request landed on GitHub's
// unauthenticated 60-per-hour API limit and silently fell back. A fallback that is
// wrong for months is not a fallback, it is a second claim. (This page shipped
// "v0.9.1" while 0.9.2 was the release, which is exactly how that starts.)
//
...
//
// It throws rather than writing whenever it cannot prove what it is about to write:
// a marker it expects is missing (the template changed shape and this script is
// silently no longer tracking anything), or the release does not actually contain
// the asset a link resolves to (the release job renamed it). A download button that
// 404s looks exactly like a working site until somebody clicks it, so that case is a
// hard failure here rather than a silent one in a visitor's browser.
...
const REPO = 'LunarWerxs/QuickDictate';
const RELEASES = `https://api.github.com/repos/${REPO}/releases/latest`;

/** Every user-facing asset a release carries, keyed by the `data-dl` value the page
 *  uses. QuickDictate ships one Windows x64 build and the filename has no version in
 *  it, so the tag alone dates the URL. Adding a platform means adding a line here AND
 *  a link in index.html; either one alone is a hard error below, which is the point. */
const DL_ASSETS = {
  'win-x64': () => 'quickdictate.exe',
};
...
/** The tag as a bare `0.9.2`, plus the names of every asset the release actually
 *  published. Refuses anything that is not a version: writing a draft name or an
 *  empty string into the page would be worse than leaving yesterday's number there. */
async function latestRelease() {
  const headers = { Accept: 'application/vnd.github+json', 'User-Agent': 'quickdictate-site-sync' };
  // GITHUB_TOKEN in Actions lifts the 60/hour anonymous limit; absent locally, which
  // is fine for a once-a-day job.
...
/**
 * Repoint every download link, and schema.org's downloadUrl, at the file in this
 * release.
 *
...
function rewriteDownloads(html, version, assets) {
  ...
  const out = html.replace(/<a\b[^>]*\sdata-dl="([a-z0-9-]+)"[^>]*>/gi, (tag, key) => {
    ...
    if (assets.size && !assets.has(file)) {
      throw new Error(
        ...
        `job renamed that asset (update DL_ASSETS) or this release did not publish that build ` +
        `(remove its link from index.html). Published: ${[...assets].join(', ') || '(none)'}`);
    }
  ...
  if (missing.length) {
    throw new Error(
      ...
      `DL_ASSETS is supposed to be offered on the page; a build that quietly stops being linked ` +
      `is a build nobody can download. Restore the link, or drop the key here.`);
  }
...
// Calling it from inside a top-level await tears the event loop down mid-flight, and
// on Windows Node aborts with a libuv assertion and exit code 127 -- so a gate meant
// to report "stale" with a 1, and a happy path meant to report success with a 0, both
// come back as a crash. Setting exitCode and letting the process end on its own gives
// the codes this script promises.
if (before === after) {
  console.log(`site version and download links already ${version}; nothing to do`);
} else {
  // Say what moved. A silent "updated" tells nobody whether the regexes still match
